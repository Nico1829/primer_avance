...
import HeadingBar from "@/components/checkout/HeadingBar";
import { useEffect, useMemo, useState } from "react";
import { useStore } from "@/store";
import { OrderItem } from "@/types";
import SummaryOrderItem from "@/components/checkout/SummaryOrderItem";
import ItemInOrderItem from "@/components/checkout/ItemInOrderItem";
...
export default function CheckoutView() {
  ...
  const addToOrderedItems = useStore((state) => state.addToOrderedItems);
  ...
  const [methodPay, setMethodPay] = useState("credit");
  ...
  function completeOrder() {
    if (
      [line1, district, city, country, methodPay, shippingMethod].includes("")
    ) {
  ...
  return (
    <div className="space-y-4 py-4 px-10">
      <Heading
        text="¡Casi Listo! Tu orden no estará completa hasta que revises y presiones el botón “completar orden”
          ...
          <div className="pl-[15px] flex flex-col gap-y-[29px] mb-[32px]">
            <div className="flex items-center gap-x-[17px]">
              <input
                ...
                id="methodPay1"
                value="qr"
                checked={methodPay === "qr"}
                onChange={(e) => setMethodPay(e.target.value)}
              />
              <label htmlFor="methodPay1">Pago con código QR</label>
            </div>
            ...
            <div className="flex items-center gap-x-[17px]">
              <input
                ...
                id="methodPay2"
                value="credit"
                checked={methodPay === "credit"}
                onChange={(e) => setMethodPay(e.target.value)}
              />
              <label htmlFor="methodPay2">Pago con tarjeta de crédito</label>
            </div>
          </div>
