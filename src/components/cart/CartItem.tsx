...
export default function CartItem({ cartItem }: CartItemProps) {
  ...
  const subTotal = cartItem.precio * cartItem.quantity;
  ...
  return (
    <div className="bg-white h-[179px] flex items-center pl-[22px] pr-[40px] justify-between py-[29px] shadow-xl">
      <div className="w-[143px] border border-slate-200 rounded-md overflow-hidden">
        <img src={cartItem.imagen} alt="imagen del producto h-[121px]" />
      </div>
      <div className="w-[640px] flex flex-col h-full justify-between py-3">
        <p className="font-bold text-2xl">{cartItem.titulo}</p>
        <div className="flex gap-x-5">
          <button className="hover:underline" onClick={() => deleteToCart(cartItem)}>Eliminar</button>
          <span>|</span>
          <button onClick={savedToSavedItems} className="hover:underline">Guardar para después</button>
        </div>
      </div>
      <div className="h-full py-3">
        ...
        >
          {cartItem.quantity > 6 ? (Array.from({ length: cartItem.quantity }, (_, i) => (
      ...
      <div className="w-[76px] h-full py-3 text-right">
        ...
        <span>{formatCurrency(cartItem.precio)}</span>
      </div>
      <div className="w-[76px] h-full py-3 text-right">
        <p className="font-bold text-base">Precio</p>
