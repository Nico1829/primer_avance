...
export default function SavedItem({ savedItem }: SavedItemProps) {
  const changeQuantitySavedItems = useStore(state => state.changeQuantitySavedItems);

  const addToCart = useStore(state => state.addToCart);

  const deleteToSavedItems = useStore(state => state.deleteToSavedItems);

  const subTotal = savedItem.precio * savedItem.quantity;
  ...
  return (
    <div className="bg-white h-[179px] flex items-center pl-[22px] pr-[40px] justify-between py-[29px] shadow-xl">
      <div className="w-[143px] border border-slate-200 rounded-md overflow-hidden">
        <img src={savedItem.imagen} alt="imagen del producto h-[121px]" />
      </div>
      <div className="w-[640px] flex flex-col h-full justify-between py-3">
        <p className="font-bold text-2xl">{savedItem.titulo}</p>
        <div className="flex gap-x-5">
          <button className="hover:underline" onClick={() => deleteToSavedItems(savedItem)}>Eliminar</button>
          <span>|</span>
          <button className="hover:underline" onClick={moveToCart}>Mover al carrito</button>
        </div>
      </div>
      <div className="h-full py-3">
        ...
        >
          {savedItem.quantity > 6 ? (Array.from({ length: savedItem.quantity }, (_, i) => (
      ...
      <div className="w-[76px] h-full py-3 text-right">
        ...
        <span>{formatCurrency(savedItem.precio)}</span>
      </div>
      <div className="w-[76px] h-full py-3 text-right">
        <p className="font-bold text-base">Precio</p>
